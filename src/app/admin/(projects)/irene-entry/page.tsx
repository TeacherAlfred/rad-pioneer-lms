                ...
                <div className="space-y-3 border-t border-slate-200 pt-4">{formData.cubs.map((cub, index) => (
                  <div key={index} className="p-3 bg-white border border-slate-100 rounded-xl space-y-2">
                    <div className="grid grid-cols-12 gap-3 items-end">
                      <div className="col-span-4"><label className="block text-[9px] font-bold text-slate-500 uppercase mb-1">Initial</label><input required maxLength={1} value={cub.cub_initial} onChange={(e) => handleCubChange(index, 'cub_initial', e.target.value.replace(/[^a-zA-Z]/g, '').slice(0, 1))} className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#0066cc] text-center uppercase" /></div>
