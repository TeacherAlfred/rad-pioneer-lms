...
export async function GET() {
  const { data, error } = await supabaseAdmin
    .from('passes')
    .select('*, leads(id, name, phone), first_session:first_session_id(id, starts_at, programme_id, programs(id, code, name)), pass_credits(id, status, enrolment_id, redeemed_at)')
...
export async function POST(req: Request) {
  try {
    ...
    const { data: enrolment, error: enrolErr } = await supabaseAdmin
      .from('enrolments')
      .insert([{ student_id: firstStudentId, session_id: firstSessionId, pass_credit_id: firstCredit.id, status: 'registered' }])
      .select()
      .single();
    ...
    const { data: fullPass, error: fetchErr } = await supabaseAdmin
      .from('passes')
      .select('*, leads(id, name, phone), pass_credits(id, status, enrolment_id, redeemed_at)')
      .eq('id', pass.id)
      .single();
...
export async function PATCH(req: Request) {
  try {
    ...
    const { data: enrolment, error: enrolErr } = await supabaseAdmin
      .from('enrolments')
      .insert([{ student_id: studentId, session_id: sessionId, pass_credit_id: redeemCreditId, status: 'registered' }])
      .select()
      .single();
    if (enrolErr) throw enrolErr;
    ...
    return NextResponse.json({ ok: true, enrolment });
  } catch (error: any) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
