...
import { unparkWipBook, type BookWithTags } from "../../reader/_actions/books";

interface ParkedWipRowProps {
  book: BookWithTags;
  onUnparked: (bookId: string) => void;
}

export default function ParkedWipRow({ book, onUnparked }: ParkedWipRowProps) {
  const [isUnparking, setIsUnparking] = useState(false);

  const handleUnpark = async () => {
    setIsUnparking(true);
    try {
      await unparkWipBook(book.id);
      onUnparked(book.id);
    } catch (error) {
      console.error("Failed to unpark book", error);
      toast.error(error instanceof Error ? error.message : "Failed to move back to the inbox.");
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-slate-100 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="font-precision text-[11px] text-slate-400 truncate">{book.author || "No author matched"}</p>
      </div>
      <button
        onClick={handleUnpark}
        disabled={isUnparking}
        title="Move back to the inbox queue"
