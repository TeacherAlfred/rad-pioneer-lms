...
import { unparkBook, type GenreReviewBook } from "../../reader/_actions/books";

interface ParkedBookRowProps {
  book: GenreReviewBook;
  onUnparked: (bookId: string) => void;
}

export default function ParkedBookRow({ book, onUnparked }: ParkedBookRowProps) {
  const [isUnparking, setIsUnparking] = useState(false);

  const handleUnpark = async () => {
    setIsUnparking(true);
    try {
      await unparkBook(book.id);
      onUnparked(book.id);
    } catch (error) {
      console.error("Failed to unpark book", error);
      toast.error(error instanceof Error ? error.message : "Failed to move back to review.");
