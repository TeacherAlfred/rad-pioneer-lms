...
import { 
  Cpu, Gamepad2, Sparkles, CheckCircle2, AlertTriangle, Share2,
  ChevronRight, ShieldCheck, Clock, Loader2, Target, Users, Plus, X,
  ChevronDown, Star, MessageSquareHeart, ChevronLeft, ArrowRight, UserPlus
} from "lucide-react";
...
export default function ReferralInvitePage() {
  ...
  // Data we need for them sharing THEIR new link
  const [newLeadId, setNewLeadId] = useState("");

  // Form State (Blank Slate)
  ...
  useEffect(() => {
    if (referrerId) fetchReferrerData();
  }, [referrerId]);

  async function fetchReferrerData() {
    try {
      const { data, error } = await supabase.from('prospects').select('name').eq('id', referrerId).single();
      if (error || !data) throw new Error("Referrer not found");
  ...
  const copyReferralLink = async () => {
    try {
      await navigator.clipboard.writeText(`${window.location.origin}/referral/${newLeadId}`);
      setShareCopied(true);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#fafcff] flex flex-col items-center justify-center">
        ...
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Verifying Referral Link...</span>
      </div>
    );
  }

  if (!referrerData && !loading) {
    return (
      <div className="min-h-screen bg-[#fafcff] flex items-center justify-center p-6">
        <div className="text-center space-y-4">
          ...
          <h2 className="text-2xl font-black uppercase italic tracking-tighter text-slate-900">Link Expired or Invalid</h2>
        </div>
      </div>
    );
  }
              ...
              <>
                <div className="text-center mb-6 sm:mb-8 border-b border-slate-100 pb-6">
                  <div className="space-y-3">
                    <div className="w-12 h-12 bg-fuchsia-50 text-fuchsia-600 rounded-full flex items-center justify-center mx-auto border border-fuchsia-100"><UserPlus size={20}/></div>
                    <h3 className="text-xl font-black text-slate-900 uppercase italic leading-tight">You've been invited by<br/><span className="text-fuchsia-600">{referrerData.name}</span>!</h3>
                    <p className="text-[10px] sm:text-xs font-bold text-emerald-600 bg-emerald-50 py-1.5 px-3 rounded-lg inline-block border border-emerald-100">
                      If you upgrade to paid, you BOTH get a free 1-on-1 lesson!
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-6 sm:mb-8 space-y-4">
                  ...
                  <button onClick={() => setWizardStep(1)} className="w-full px-8 py-4 sm:py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase tracking-widest text-xs sm:text-sm transition-all shadow-xl shadow-blue-600/20 flex items-center justify-center gap-2 group hover:-translate-y-1">
                ...
                {wizardStep === 1 && (
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input required type="text" value={parentName} onChange={e => setParentName(e.target.value)} className="w-full mt-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:border-blue-500 shadow-sm" />
                    </div>
                    <div>
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Email Address *</label>
                      <input required type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full mt-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:border-blue-500 shadow-sm" />
                    </div>
                    <div>
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Phone Number *</label>
                      <input required type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="w-full mt-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:border-blue-500 shadow-sm" />
                    </div>
