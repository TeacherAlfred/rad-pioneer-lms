...
export default function SequenceViewer({ cards, onComplete, formatText, onCardChange }: SequenceViewerProps) {
  ...
  // Safely notify the parent when a card changes so Vocab words unlock.
  useEffect(() => {
  ...
  const renderMedia = (url: string, title: string) => {
    ...
    return (
      <div 
        ...
        className="relative w-full aspect-video lg:aspect-square flex items-center justify-center rounded-[32px] overflow-hidden group cursor-pointer bg-black/5"
      >
        ...
        <motion.img 
          ...
          src={url} 
          alt={title} 
          className="relative z-10 w-full h-full object-cover sm:object-contain drop-shadow-xl p-4 transition-transform duration-500 group-hover:scale-105" 
        />

        <div className="absolute bottom-4 right-4 z-20 opacity-0 group-hover:opacity-100 transition-opacity p-2 bg-black/50 backdrop-blur-md rounded-full text-white shadow-lg">
          <Maximize2 size={16} />
        </div>
      </div>
    );
  ...
  // CRITICAL FIX: Removed `scale` from framer-motion variants.
  // Using scale causes CSS transform bugs that displace the clickable hitboxes of child elements!
  const slideVariants = {
  ...
  return (
    <>
      <div className="w-full max-w-5xl mx-auto bg-black/5 rounded-[32px] md:rounded-[48px] overflow-hidden flex flex-col relative border border-slate-200/20 shadow-md">
        
        {/* --- HEADER & PROGRESS BAR --- */}
        <div className="p-6 md:p-8 shrink-0">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3 text-inherit opacity-70">
              <div className="p-2 rounded-xl bg-black/5 border border-white/10">
                <Gamepad2 size={18} />
              </div>
              <span className="text-[10px] font-black uppercase tracking-widest">Active Mission Sequence</span>
            </div>
            <span className="text-[10px] font-black uppercase tracking-widest opacity-50 bg-black/5 px-4 py-2 rounded-full border border-white/5">
        ...
        <div className="flex-1 relative overflow-hidden px-6 md:px-12 pt-4 pb-12 flex flex-col justify-center min-h-[300px]">
            ...
            >
              <div className={`grid gap-8 lg:gap-12 items-center ${currentCard.media_url ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 max-w-3xl mx-auto text-center'}`}>
                ...
                <div className={`space-y-6 p-2 ${!currentCard.media_url && 'flex flex-col items-center'}`}>
                  <h2 
                    className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter leading-[1.1] pb-2 text-inherit"
                    dangerouslySetInnerHTML={{ __html: formatText ? formatText(currentCard.title) : currentCard.title }}
                  />

                  <div 
                    className="text-sm md:text-base leading-loose font-medium pb-2 text-inherit opacity-80"
        ...
        {/* CRITICAL FIX: Removed overlapping floating absolute buttons. Using sturdy native buttons in the footer. */}
        <div className="p-4 md:p-6 border-t border-slate-200/20 bg-black/5 flex items-center justify-between">
          <button
            onClick={handlePrev}
            disabled={isFirstCard}
            className={`w-12 h-12 flex items-center justify-center rounded-full transition-all ${isFirstCard ? 'opacity-30 cursor-not-allowed text-inherit' : 'bg-black/10 hover:bg-black/20 text-inherit shadow-sm hover:scale-105'}`}
          >
            <ChevronLeft className="w-6 h-6 mr-0.5" />
          </button>

          <span className="text-[10px] md:text-xs font-black uppercase tracking-widest opacity-50">
            Card {currentIndex + 1} of {sortedCards.length}
          </span>

          <button
            onClick={handleNext}
            className={`flex items-center justify-center gap-2 transition-all hover:scale-105 shadow-md ${
                isLastCard 
                ? 'h-12 px-6 rounded-full bg-emerald-500 text-white border border-emerald-400 font-black uppercase tracking-widest text-xs hover:bg-emerald-400' 
                : 'w-12 h-12 rounded-full bg-blue-600 text-white hover:bg-blue-500'
            }`}
          >
            {isLastCard ? (
              <><span>Complete</span><CheckCircle2 className="w-5 h-5" /></>
            ) : (
