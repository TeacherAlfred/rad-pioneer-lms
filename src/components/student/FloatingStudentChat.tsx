...
export default function FloatingStudentChat() {
  ...
  useEffect(() => {
    async function initChat() {
      try {
        ...
        // Fetch profile to get assigned teacher AND linked parent
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      if (data) {
        setMessages(data);
        // Count unread messages sent BY the teacher TO the student
  ...
  return (
    <div className="fixed bottom-6 right-6 lg:bottom-10 lg:right-10 z-[9999] flex flex-col items-end">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#020617] custom-scrollbar">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
                  ...
                  <p className="text-xs font-bold text-slate-400">No messages yet.<br/>Say hi to your coach!</p>
                </div>
              ) : (
      ...
      <motion.button
        ...
        className="relative w-14 h-14 md:w-16 md:h-16 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full shadow-[0_0_20px_rgba(59,130,246,0.5)] flex items-center justify-center text-white border border-blue-400/50"
      >
