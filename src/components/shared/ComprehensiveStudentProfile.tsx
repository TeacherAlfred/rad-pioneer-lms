...
import ItineraryManager from "@/components/profile/ItineraryManager";
...
export default function ComprehensiveStudentProfile({ studentId, role }: ProfileProps) {
  ...
  // NEW: Dynamic SQL Attendance State
  const [attendanceStats, setAttendanceStats] = useState({ rate: 100, attended: 0, scheduled: 0 });
  ...
  async function fetchStudentData() {
    try {
      ...
      // --- DYNAMIC SQL ATTENDANCE CALCULATION ---
      ...
      if (scheduleData && scheduleData.length > 0) {
        const scheduledCount = scheduleData.length;
        const attendedCount = scheduleData.filter(l => 
          ['attended', 'present', 'late'].includes((l.attendance_status || '').toLowerCase())
        ).length;
        
        const rate = Math.round((attendedCount / scheduledCount) * 100);
        setAttendanceStats({ rate, attended: attendedCount, scheduled: scheduledCount });
      } else {
        setAttendanceStats({ 
          rate: meta.attendance_rate || 100, 
          attended: meta.lessons_attended || 0, 
          scheduled: meta.lessons_scheduled || 0 
        });
      }
    ...
    <div className={`min-h-screen bg-[#020617] text-white p-6 lg:p-12 font-sans selection:bg-blue-500/30 overflow-x-hidden ${isBulkMode ? 'pb-32' : ''}`}>
      ...
      <div className="max-w-[1400px] mx-auto space-y-8">        
             ...
             <div className="bg-white/5 p-5 rounded-3xl border border-white/10 flex flex-col justify-center min-w-[140px] relative group">
               ...
               <div className="flex items-end gap-2">
                 <p className="text-3xl font-black italic text-emerald-400">
                   {attendanceStats.rate}<span className="text-xl">%</span>
                 </p>
                 <span className="text-[10px] font-bold text-slate-500 mb-1">
                   {attendanceStats.attended}/{attendanceStats.scheduled}
                 </span>
               </div>
               <p className="text-[8px] font-bold text-slate-500/70 uppercase tracking-widest mt-1">Calculated to Yesterday</p>
               <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {/* LEFT COLUMN: Pioneer Config, Academics, Comms */}
          <div className="space-y-8 min-w-0">
            ...
            <div className="bg-[#0f172a] rounded-[40px] border border-white/5 shadow-2xl flex flex-col h-[500px] overflow-hidden">
          ...
          {/* RIGHT COLUMN: Itinerary Manager */}
          <div className="space-y-6 min-w-0">
            <ItineraryManager studentId={student.id} />
          </div>
