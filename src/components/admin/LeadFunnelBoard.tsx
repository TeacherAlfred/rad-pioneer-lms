...
// yyyy-mm-dd in the same timezone every other date on this page displays
// in - matches <input type="date">'s own value format, so it can be
// compared to dateFrom/dateTo with plain string comparison.
function leadDateKey(iso: string | null | undefined): string | null {
  if (!iso) return null;
...
export type LeadFunnelScope = 'all' | 'ads';

// Shared by both /admin/lead-funnel/list (scope="all") and
// /admin/lead-funnel/ads (scope="ads") - the two pages are otherwise
// identical (same table, same send/edit/view tooling), so "ads" is just a
// baseRows narrowing (ad_id must be set) plus a campaign drill-down on top,
// not a separate implementation to keep in sync.
export default function LeadFunnelBoard({ scope }: { scope: LeadFunnelScope }) {
  ...
  // "When did this lead first enter the system" - created_at, yyyy-mm-dd
  // strings straight out of <input type="date">. Empty = no bound. Narrows
  // statsRows (see below), not just the visible table, so every stat and
  // the By Campaign breakdown answer "how many NEW leads in this range" -
  ...
  // Hours; 0 = off. Hides leads who were actually sent a message within
  ...
  // Ad Campaigns scope narrows to ad-attributed leads before anything else
  // runs - every stat, filter option, and row below is computed off this,
  // never off the raw fetch, so organic/direct leads never leak into the
  // ads section even transiently.
  const baseRows = useMemo(() => scope === 'ads' ? rows.filter(r => r.ad_id) : rows, [rows, scope]);

  // Date range narrows on top of baseRows (before the inhouse split below),
  // so it's not just a table filter - every stat tile and the By Campaign
  // breakdown answer "how many NEW leads in this window" too.
  const dateRows = useMemo(() => {
    if (!dateFrom && !dateTo) return baseRows;
    return baseRows.filter(r => {
      const key = leadDateKey(r.created_at);
      if (!key) return false;
  ...
  // ad_headline is the friendly label Meta gives the creative - falls back
  // to the raw ad_id for the (rare) lead where that never got captured.
  const campaignOptions = useMemo(
    () => Array.from(new Set(baseRows.filter(r => r.ad_id).map(r => r.ad_headline || r.ad_id!))).sort(),
    [baseRows]
  );
  ...
  const filteredRows = useMemo(() => {
    ...
    return source
      .filter(r => {
        ...
        if (scope === 'ads' && campaignFilter !== 'all' && (r.ad_headline || r.ad_id) !== campaignFilter) return false;
        if (queueOnly && !queuePositions.has(r.id)) return false;
        if (hideReceivedHours > 0 && isWithinHours(r.last_received_at, hideReceivedHours)) return false;
        if (q) {
    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-10">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">{scope === 'ads' ? 'Ad Campaign Leads' : 'Lead Funnel'}</h1>
            <p className="text-sm text-slate-500 mt-1">
              {scope === 'ads'
                ? 'Only leads with ad attribution (leads.ad_id set) - broken out by campaign, isolated from organic/direct leads so you can work this list on its own.'
                : 'Everyone in the `leads` table - WhatsApp bot, Irene voting consent, and warm-list imports. Not the registrations/profiles pipeline at /admin/leads.'}
            </p>
          </div>
          <div className="flex items-center gap-4 shrink-0">
            {scope === 'ads' ? (
              <Link href="/admin/lead-funnel/list" className="inline-flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600">
                <Users size={14} /> Full Lead Funnel
              </Link>
            ) : (
              <Link href="/admin/lead-funnel/ads" className="inline-flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600">
                <Megaphone size={14} /> Ad Campaigns
              </Link>
            )}
            <Link href="/admin/lead-funnel/merge" className="inline-flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600">
          ...
          <>
            <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-4 flex flex-wrap items-center gap-3">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5 shrink-0">
                <CalendarClock size={13} /> First entered the system
              </span>
              <input
                type="date"
                value={dateFrom}
                ...
                className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-slate-400"
              />
              ...
              />
              {(dateFrom || dateTo) && (
                <button onClick={() => { setDateFrom(''); setDateTo(''); }} className="text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600">
                  Clear
                </button>
              )}
              <span className="text-xs text-slate-400 ml-auto">
                {(dateFrom || dateTo)
                  ? `Every stat and breakdown below is scoped to leads created ${dateFrom || 'the beginning'} – ${dateTo || 'now'}.`
                  : 'Filters every stat and breakdown below by lead created date - leave blank for all-time.'}
              </span>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-6">
              <StatCard icon={Users} label={(dateFrom || dateTo) ? 'Leads in Range' : 'Total Leads'} value={stats.total} />
              ...
              {scope === 'ads' ? (
                <StatCard icon={Megaphone} label="Campaigns" value={campaignOptions.length} accent="text-indigo-600" />
              ) : (
            ...
            {scope === 'ads' ? (
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                ...
                <CampaignBreakdownCard data={stats.byAd} active={campaignFilter} onSelect={setCampaignFilter} />
              </div>
            ) : (
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <BreakdownCard title="By Status" data={stats.byStatus} formatLabel={statusLabel} />
            ...
            {scope === 'all' && Object.keys(stats.byAd).length > 0 && (
              <div className="mb-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-4 flex flex-wrap gap-3 items-center">
              ...
              {scope === 'ads' ? (
                <select value={campaignFilter} onChange={e => setCampaignFilter(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none">
              ...
              <label className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-500">
                ...
                <select
                  value={hideReceivedHours}
                  onChange={e => {
                    const hours = Number(e.target.value);
                    setHideReceivedHours(hours);
                    // Someone already ticked who's now hidden must not stay
                    // selected - they'd still get the next bulk send.
                    if (hours > 0) {
                      const hidden = new Set(rows.filter(r => isWithinHours(r.last_received_at, hours)).map(r => r.id));
                      setSelectedIds(prev => new Set([...prev].filter(id => !hidden.has(id))));
                    }
                  }}
                  className={`bg-slate-50 border rounded-xl px-3 py-2 text-sm font-normal normal-case tracking-normal outline-none ${hideReceivedHours > 0 ? 'border-amber-400 text-amber-700' : 'border-slate-200'}`}
                >
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">By Campaign (click to drill in)</h3>
        {active !== 'all' && (
          <button onClick={() => onSelect('all')} className="text-[10px] font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-700">Clear</button>
        )}
      </div>
      <div className="space-y-2">
        {entries.map(([key, count]) => (
          <button
            ...
            className={`w-full flex items-center gap-3 rounded-lg -mx-1 px-1 py-0.5 transition-colors ${active === key ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}
          >
            <span className={`text-xs w-40 shrink-0 truncate text-left ${active === key ? 'text-indigo-700 font-bold' : 'text-slate-600'}`} title={key}>{key}</span>
            <div className="flex-1 bg-slate-50 rounded-full h-2 overflow-hidden">
              <div className={`h-full rounded-full ${active === key ? 'bg-indigo-500' : 'bg-slate-800'}`} style={{ width: `${(count / max) * 100}%` }} />
            </div>
